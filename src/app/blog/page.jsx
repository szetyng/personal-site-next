            ...
            {posts.map((post) => (
              <Fragment key={post.slug}>
                <Link href={`/blog/${post.slug}`} className="block group">
                  <article className="border-l-4 border-transparent hover:border-mint-green hover:pl-3 py-4 transition-all h-full">
                    ...
                    <h2 className="text-xl font-bold mt-2 group-hover:text-mint-green transition-colors">
                      {post.title}
                    </h2>
                    {post.subtitle && (
